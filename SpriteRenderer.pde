class SpriteRenderer {
  ...
  private void dibujarFrame(int fila) {
    PImage frame = spriteSheet.get(0, fila * altoFrame, anchoFrame, altoFrame);

    boolean reflejar = objeto instanceof Personaje && ((Personaje)objeto).isMirandoIzquierda();

    if (reflejar) {
      pushMatrix();
      translate(objeto.getPosicion().x, objeto.getPosicion().y);
      scale(-1, 1);
      image(frame, 0, 0, -objeto.getTamanio().x, objeto.getTamanio().y); // Negativo para evitar distorsión
      popMatrix();
    } else {
      image(frame, objeto.getPosicion().x, objeto.getPosicion().y, objeto.getTamanio().x, objeto.getTamanio().y);
    }
  }

  private void moverSprite(int fila) {
    int columna = frameActual % columnas;
    PImage frame = spriteSheet.get(columna * anchoFrame, fila * altoFrame, anchoFrame, altoFrame);
    ...
    if (reflejar) {
      ...
      translate(objeto.getPosicion().x, objeto.getPosicion().y);
      scale(-1, 1);
      image(frame, 0, 0, -objeto.getTamanio().x, objeto.getTamanio().y);
      popMatrix();
    } else {
      image(frame, objeto.getPosicion().x, objeto.getPosicion().y, objeto.getTamanio().x, objeto.getTamanio().y);
    }
